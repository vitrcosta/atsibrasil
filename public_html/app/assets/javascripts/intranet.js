...
$(function() {
  $(document).on( 'change','.field_galeria', function (){
    ...
    '<span class="btn btn-primary">Arquivo… <input class="fileupload campos_intranet field_galeria" style="display: none;" onchange="galeria_change_file_append(this,' +num_blocks+ ')" type="file" name="post[galleries_attributes][' +num_blocks+ '][image]" id="post_galleries_attributes_' +num_blocks+ '_image"><div class="conta_quantos_botao_file"></div></span></label><input type="text" name="galeria_text-file-' +num_blocks+ '" class="form-control text_file_galeria" value="Nenhum arquivo selecionado" readonly=""></div>');
  });
});

function galeria_change_file_append(field,num_blocks){
  var text_name = "galeria_text-file-" + num_blocks;
  document.getElementsByName(text_name)[0].value = field.value.replace(/\\/g, '/').replace(/.*\//, '');
}
